...
class SmimeMail extends Base implements GatewayInterface, MessageDraftFactoryInterface
{
    /**
     * SMTP settings cache
     * @var array
    ...
    public function send(Message $objMessage, array $arrTokens, $strLanguage = '')
    {
        ...
        if (isset($GLOBALS['TL_HOOKS']['SMimeGatewaySend']) && is_array($GLOBALS['TL_HOOKS']['SMimeGatewaySend']))
		{
			foreach ($GLOBALS['TL_HOOKS']['SMimeGatewaySend'] as $callback)
			{
				$this->import($callback[0]);
				$this->{$callback[0]}->{$callback[1]}($objEmail,$arrTokens);
			}
		}
        ...
        // get private key
        $objFileModelPk = \FilesModel::findByUuid($this->objModel->smime_pk);
        
        // get cert
        $objFileModelCert = \FilesModel::findByUuid($this->objModel->smime_cert);

        $smimeSigner = \Swift_Signers_SMimeSigner::newInstance();
        $smimeSigner->setSignCertificate(
            TL_ROOT . '/'.$objFileModelCert->path, 
            array(
                TL_ROOT . '/' . $objFileModelPk->path,
                $this->objModel->smime_pk_password,
            )
        );
        $objEmail->attachSigner($smimeSigner);
